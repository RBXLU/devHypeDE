...
fn run() -> anyhow::Result<()> {
    ...
    // В сеансе на видеокарте выйти больше неоткуда: ни другого рабочего стола
    // вокруг, ни окна, которое можно закрыть. Поэтому пути наружу называются
    // сразу, а не остаются в документации.
    if backend == BackendKind::Drm {
        info!("выход: Super+Shift+E · консоли: Ctrl+Alt+F1…F12 · аварийно: Ctrl+Alt+Backspace");
    }
