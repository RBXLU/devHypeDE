...
///
/// Порядок в списке — от переднего слоя к заднему, поэтому обои добавляются
/// последними.
...
{
    let mut elements: Vec<Frame<R>> = match space_render_elements(renderer, [space], output, 1.0) {
